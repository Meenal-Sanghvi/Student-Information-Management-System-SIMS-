...
def export_data():
    ...
    messagebox.showinfo('Success','Data is saved successfully in the Excel File')
def toplevel_data(title,buttton_text,command):
    ...
    student_button = ttk.Button(screen, text=buttton_text,command=command)
...
def connect_database():
    def connect():
        ...
        try:
            ...
            messagebox.showinfo('Error','Invalid Details',parent=connectWindow)
            return
        try:
...
style=ttk.Style()
style.configure('Treeview',rowheight=40,font=('arial',12,'bold'),foreground='black',background='white',fieldground='white')
style.configure('Treeview.Heading',font=('aerial',14,'bold'),foreground='red')
